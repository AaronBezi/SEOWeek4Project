...
class DocumentAnalysis(db.Model):
    ...
    content_hash = db.Column(db.String(64),index=True)      #prevents identical notes from being generated
    time_created = db.Column(db.DateTime(timezone=True), nullable=False, default=datetime.utcnow)


#Schema for the books generated by the google boosk api
class Book(db.Model):
    __tablename__ = "books"
...
#creates dic anaylis object takes in note_id doc_analysis object contaiing doc_analysis features, hash code, and embedding
def create_Doc_Analysis(note_id,doc_analysis,h, embedding_gen):
    ...
    return DocumentAnalysis(note_id=note_id,subject=doc_analysis.subject, topics = doc_analysis.topics,
                            keywords = doc_analysis.keywords, academic_level = doc_analysis.academic_level,
                            summary = doc_analysis.summary, embedding = embedding_gen, content_hash = h )
